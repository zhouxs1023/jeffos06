...
static unsigned char IP[4] = { 192, 0, 0, 3 }; 
...
packet_data *alloc_buffer_data(uint size)
{
    ...
    os_AcquireSem(sem_ring);
    if(!p_next){
        os_ReleaseSem(sem_ring);
        oprintf("ne2000: !!! out of packet ringbuffers (recv)\n");
        return NULL;
    }
    P = p_next;
    p_next = P->next;
...
/*    mh.flags = 0;
    ...
    mh.size = 4;
    mh.data = &mh.flags;
    if(port_send(&mh) != 4) oprintf("free_buffer: blargh!"); */
}
...
void xmit(pbuf *P)
{
    ...
    mh.size = 4;
    mh.data = &P;
    if((i = port_send(&mh)) != 4) oprintf("xmit: blargh! %d\n",i);
...
void print_arp(unsigned char *p)
{
    ...
    oprintf("  ARP: ");
    t = htons(arp->arp_op);
    if(t == ARP_OP_REQUEST) oprintf("req ");
    else if(t == ARP_OP_REPLY) oprintf("rep ");
    else oprintf("??? ");

    b = (unsigned char *) &(arp->arp_enet_sender);
    oprintf("source:  %X:%X:%X:%X:%X:%X ",b[0],b[1],b[2],b[3],b[4],b[5]);
    ...
    oprintf("%X:%X:%X:%X:%X:%X ",b[0],b[1],b[2],b[3],b[4],b[5]);
...
void xxreceive(void *cb, packet_data *packet)
{
    ...
    if((i = port_send(&msg)) != 4){
        oprintf("receive: dispatch send failed %d\n",i);
    ...
    for(;;){
        ...
        if(size >= 8){
            ...
            case NET_CONNECT:
                ...
                oprintf("ne2000: routing udp:%d -> blt:%d\n",
                       cmd->port, msg.src);
...
int main(void)
{
    ...
    sem_ring = os_CreateSem(1);
    mutex = os_CreateSem(1);
...
/*    while((port_console = namer_find(nh,"console")) < 1)
    ...
    init_ring();
    TheSNIC.iobase = 0;
    nic_init(&TheSNIC, 0x340, prom, NULL);

    oprintf("ne2000: irq %d @ 0x300 mac = %X:%X:%X:%X:%X:%X\n",
           snic_irq,prom[0],prom[1],prom[2],prom[3],prom[4],prom[5]);    

    nic_register_notify(&TheSNIC,receive,NULL);
    
    oprintf("ne2000: starting sender, dispatcher, and control\n");

    os_thread(sender, 0);
    os_thread(dispatcher, 0);
    os_thread(control, 0);
